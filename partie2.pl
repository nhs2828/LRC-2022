...
acquisition_prop_type1([X|Abi],[Y|Abi1],Tbox) :- 
  traitement_Abox(not(X),Y,Tbox), acquisition_prop_type1(Abi,Abi1,Tbox).

%acquisition_prop_type2
acquisition_prop_type2([],[],_).
acquisition_prop_type2([and(X1,X2)|Abi],[Y|Abi1],Tbox) :- 
  traitement_Abox(and(X1,X2),Y,Tbox), acquisition_prop_type2(Abi,Abi1,Tbox).
